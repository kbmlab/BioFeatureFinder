...
if args.ensembl_to_ucsc:
    ##Convert Ensembl to UCSC
    ens = pd.concat(
        BedTool(args.conv_file).to_dataframe(iterator=True, chunksize=10000),
    ...
    ens.to_csv(args.out, compression='gzip', sep='\t', index=False,
               header=False)

elif args.ucsc_to_ensembl:
    ##Convert UCSC to Ensembl
    ucsc = pd.concat(
        BedTool(args.conv_file).to_dataframe(iterator=True, chunksize=10000),
        ignore_index=True).dropna()
    ucsc[ucsc.columns[0]] = ucsc[ucsc.columns[0]].str.replace('chrM', 'chrMT')
    ucsc[ucsc.columns[0]] = ucsc[ucsc.columns[0]].apply(
        lambda x: x.split('chr')[1])
    columns = ucsc.select_dtypes(include=['float64']).columns.tolist()
    ucsc[columns] = ucsc[columns].astype(int)
    ucsc.to_csv(args.out, compression='gzip', sep='\t', index=False,
                header=False)

else:
    print(
        "No conversion option selected. Please use --ucsc-to-ensembl or --ensembl-to-ucsc.")
