...
parser.add_argument('-cs', '--conservation_scores', dest="con_files", default=False,
...
##remove the nonchromossomal annotation from the file. To do this, just uncomment the lines X-Z in the code

#if args.genome_browser == False:
#    df['start'] = (df['start']-1).astype(int)
#    df['end'] = df['end'].astype(int)
#elif args.genome_browser == True:
#    df['start'] = (df['start']-1).astype(int)
#    df['end'] = df['end'].astype(int)
...
def get_transcript_id(df):
    a = df.copy()
    a['transcript_id'] = a['name'].apply(lambda x: x.split('transcript_id_')[1])
...
single_transcripts = single_exons_bed['name'].apply(lambda x: x.split('transcript_id_')[1])
...
def get_upstream_and_downstream_exons(transcript_id):
    ...
    strand_counts = transcript_all_exons['strand'].value_counts().shape[0] #how many strands
    strand_id = pd.DataFrame(transcript_all_exons['strand']).reset_index()['strand'][0] #which strand
    
    all_exons_bedtool = BedTool.from_dataframe(transcript_all_exons).sort()
    
    if (strand_counts == 1) and (strand_id == "+"):
        if exon_count == 2:
            last_exons_bedtool = BedTool.from_dataframe(sorted_search(last_exons_df, transcript_id)).sort()
            first_exons_bedtool = BedTool.from_dataframe(sorted_search(first_exons_df, transcript_id)).sort()
        ...
        if exon_count >= 3:
            last_exons_bedtool = BedTool.from_dataframe(sorted_search(last_exons_df, transcript_id)).sort()
            first_exons_bedtool = BedTool.from_dataframe(sorted_search(first_exons_df, transcript_id)).sort()
            middle_exons_bedtool = BedTool.from_dataframe(sorted_search(middle_exons_df, transcript_id)).sort()
...
## Functions for getting upstream and downstream features

def get_upstream_features(table):

    cat = pd.read_table(table, header=None)
    cat['tag'] = cat[3]#+'_'+cat[9] ##The second part concatenates the upstream exon ID, but for simplicity we will not use that portion. If you want this information just uncoment the line and make the necessary changes to the script.
    ...
    df_introns = pd.concat([df_introns_p,df_introns_m]
                          ).drop_duplicates().reindex(columns=df_introns_p.columns).dropna()
...
if args.keep_bed == True:
    ...
    save_bed(up_middle_intron, str(args.prefix)+'.upstream_middle_introns.bed.gz')
    ...
    #Dn middle
    save_bed(dn_middle_exon, str(args.prefix)+'.downstream_middle_exons.bed.gz')
    save_bed(dn_middle_intron, str(args.prefix)+'.downstream_middle_introns.bed.gz')
    ...
    #Dn first
    save_bed(dn_first_exon, str(args.prefix)+'.downstream_first_exons.bed.gz')
    save_bed(dn_first_intron, str(args.prefix)+'.downstream_first_introns.bed.gz')
    ...
    save_bed(up_last_intron, str(args.prefix)+'.upstream_last_introns.bed.gz')
...
def nuc_cont(bedtool):
    ...
    nuccont_df['%A'] = nuccont_df.apply(lambda x: (float(x['%A'])/float(x['length'])), 1).round(5)
    nuccont_df['%C'] = nuccont_df.apply(lambda x: (float(x['%C'])/float(x['length'])), 1).round(5)
    nuccont_df['%G'] = nuccont_df.apply(lambda x: (float(x['%G'])/float(x['length'])), 1).round(5)
...
def get_cpg_islands(bedtool):
    ...
    cpg_counts = pd.concat(bedtool.intersect(cpg, c=True).to_dataframe(iterator=True, chunksize=10000
...
def get_data(df, name, matrix):
    ...
    if name.find("_exon") != -1 and name.find('single') == -1:
        if args.ese_list == False:
            ...
            a['ese_count'] = a['seq'].apply(lambda x: sum(x.upper().count(y.upper()) for y in (ese_list)))
            
        if args.ess_list == False:
            pass
        elif args.ess_list != False:
            a['ess_count'] = a['seq'].apply(lambda x: sum(x.upper().count(y.upper()) for y in (ess_list)))
            
        if args.custom_seq_ex == False:
            pass
        elif args.custom_seq_ex != False:
            a['custom_seq_exon'] = a['seq'].apply(lambda x: sum(x.upper().count(y.upper()) for y in (custom_seq_ex)))
            
    if name.find("_intron") != -1:
        if args.ise_list == False:
            pass
        elif args.ise_list != False:
            a['ise_count'] = a['seq'].apply(lambda x: sum(x.upper().count(y.upper()) for y in (ise_list)))
            
        if args.iss_list == False:
            pass
        elif args.iss_list != False:
            a['iss_count'] = a['seq'].apply(lambda x: sum(x.upper().count(y.upper()) for y in (iss_list)))
...
if args.debug != False:
    first_exons_bed = first_exons_bed[first_exons_bed['name'].isin(dn_first_3ss['name'])]
    middle_exons_bed = middle_exons_bed[middle_exons_bed['name'].isin(dn_middle_3ss['name'])]
    last_exons_bed = last_exons_bed[last_exons_bed['name'].isin(up_last_3ss['name'])]
...
frames_first = list((first_exons_bed, 
                     dn_first_exon, dn_first_intron, dn_first_3ss, dn_first_5ss))
names_first = list(('first_exons', 
                    'dn_first_exon', 'dn_first_intron', 'dn_first_3ss', 'dn_first_5ss'))

matrix_middle = pd.DataFrame(middle_exons_bed['name'])
frames_middle = list((middle_exons_bed,
                      dn_middle_exon, dn_middle_intron, dn_middle_3ss, dn_middle_5ss,
                      up_middle_exon, up_middle_intron, up_middle_3ss, up_middle_5ss))
names_middle = list(('middle_exons',
                     'dn_middle_ex', 'dn_middle_int', 'dn_middle_3ss', 'dn_middle_5ss',
                     'up_middle_ex', 'up_middle_int', 'up_middle_3ss', 'up_middle_5ss'))
